...
camera_height = 960
frame_rate = 30
camera_rotate = True  # Set to True if the camera is rotated
camera_hflip = False   # Set to True if the camera image is horizontally flipped
camera_vflip = True  # Set to True if the camera image is vertically flipped
...
def detect_aruco_markers(image_path=None, camera_id=0, use_camera=False, marker_length=0.05):
    """
    ...
    if use_camera:
        ...
        while True:
            ...
            # Apply camera orientation adjustments
            if camera_rotate:
            ...
            process_frame(frame, detector, marker_length)
        ...
        if image is None:
            ...
            return
        process_frame(image, detector, marker_length)
        cv2.waitKey(0)
        cv2.destroyAllWindows()

def process_frame(frame, detector, marker_length):
    """
