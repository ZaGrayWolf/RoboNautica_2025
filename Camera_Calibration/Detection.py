...
def pnp_dlt(objPoints, imgPoints, cameraMatrix, distCoeff):
    ...
    imgPointsNormalized = cv2.undistortPoints(imgPoints, cameraMatrix, distCoeff)

    # Estimate Homography
    H, _ = cv2.findHomography(objPoints, imgPointsNormalized)

    # Estimate Rotation and Translation
    K_inv = np.linalg.inv(cameraMatrix)
    H_inv = np.linalg.inv(H)
    R1 = np.dot(K_inv, H)
    R2 = np.dot(R1, cameraMatrix)

    # Add a homogeneous coordinate to represent translation
    R2 = np.vstack((R2, [0, 0, 0, 1]))
