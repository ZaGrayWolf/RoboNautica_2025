...
def pnp_dlt(objpoints, imgPoints, cameraMatrix, distCoeff):
  """
  Solves the Perspective-n-Point (PnP) problem using DLT algorithm.

  Args:
      objpoints: A list of object points (3D coordinates) in world frame (Nx3 numpy array).
      imgPoints: A list of image points (2D coordinates) in image frame (Nx2 numpy array).
      cameraMatrix: Camera calibration matrix (3x3 numpy array).
      distCoeff: Distortion coefficients for the camera (optional, default None).

  Returns:
      A tuple containing the rotation vector (3x1 numpy array) and translation vector (3x1 numpy array), 
      or None on failure.
  """
  ...
  if distCoeff is not None:
    _ = cv2.undistortPoints(
    imgPoints.reshape(-1, 1, 2), cameraMatrix, distCoeff)


  # Assuming there's a single marker with 4 corners (modify based on your needs)
  num_points = len(imgPoints)
  if num_points != 4:
    print(f"Expected 4 image points, got {num_points}.")
    return None

  # Create a 4x4 homogeneous transformation matrix for each point
  A = np.zeros((num_points * 3, 4))
  for i in range(num_points):
    x, y = imgPoints[i]
    X, Y, Z = objpoints[i]
    row1 = np.array([0, -Z*x, Z*y, X])
    row2 = np.array([Z*x, 0, -Z*y, Y])
    row3 = np.array([-y, x, 0, -1])
    A[i * 3: (i + 1) * 3] = np.vstack((row1, row2, row3))

  # Perform SVD on the A matrix
  U, S, Vt = np.linalg.svd(A)

  # Extract the last column of Vt (right singular vector)
  last_column = Vt[-1]
  ...
  rotation_3x3 = reshaped_matrix[:, :3]
  translation_3x1 = reshaped_matrix[:, 3:]

  # Refine rotation using SVD (optional for robustness)
  U, D, Vt = np.linalg.svd(rotation_3x3)
  rotation_matrix = np.dot(U, Vt)

  # Return rotation vector and translation vector
  rvec, _ = cv2.Rodrigues(rotation_matrix)
  tvec = np.dot(inv_camera_matrix, translation_3x1)
