...
def pnp_dlt(objpoints, imgPoints, cameraMatrix, distCoeff):
    ...
    objpoints_normalized = objpoints - np.mean(objpoints, axis=0)
    imgPoints_normalized = imgPoints - np.mean(imgPoints, axis=0)

    # Compute the cross-covariance matrix
    H = np.dot(objpoints_normalized.T, imgPoints_normalized)

    # Perform Singular Value Decomposition (SVD) on H
    U, S, Vt = np.linalg.svd(H)

    # Ensure U and Vt are both of shape (3, 3)
    U = U[:, :3]
    Vt = Vt[:3, :]
    ...
    t = np.mean(imgPoints, axis=0) - np.dot(np.mean(objpoints, axis=0), R.T)
