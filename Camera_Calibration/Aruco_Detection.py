...
def undistort_points(camera_matrix, distortion_coeffs, image_points):
    ...
    #Distortion Coefficients: [[ 3.48533793e+00  8.70974618e+01  2.37235072e-01 -1.36174455e-019.91264335e+02]]
...
def pnp_dlt(objPoints, imgPoints, cameraMatrix, distCoeff):
    ...
    try:
        ...
        for _ in range(3):  # Perform 3 iterations (adjust as needed)
            # Project 3D points onto the image plane using current pose estimation
            markerLength_i = markerLength * 1.5
            _, rvec = cv2.Rodrigues(R.astype(np.float32))

            axisPoints = np.float32([[0, 0, 0], [markerLength_i, 0, 0], [0, markerLength_i, 0], [0, 0, -markerLength_i]])
            
            # Project points, handle exceptions
            try:
                projected_imgPoints, _ = cv2.projectPoints(axisPoints, rvec, t, cameraMatrix, distCoeff)
            except Exception as e:
                print("Error in cv2.projectPoints:", e)
                return None, None
            ...
            for i in range(objPoints.shape[0]):
                X, Y, Z, _ = objPoints_hom[i, :]
                u, v = projected_imgPoints[i, 0], projected_imgPoints[i, 1]
                J[2 * i, :] = [-X/Z, -Y/Z, -1/Z, 0, 0, 0]
                J[2 * i + 1, :] = [0, 0, 0, -X/Z, -Y/Z, -1/Z]
